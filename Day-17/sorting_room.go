...
func ExtractFancyNumber(fnb FancyNumberBox) int {
    switch fnb.(type) {
        case FancyNumber:
            fancyNumberInt, _ := strconv.Atoi(fnb.Value())
    		return fancyNumberInt
        default:
        	return 0
    }
...
func DescribeFancyNumberBox(fnb FancyNumberBox) string {
    switch val := fnb.(type) {
        case FancyNumber:
        	return fmt.Sprintf("This is a fancy box containing the number %.1f", float64(ExtractFancyNumber(val)))
        default:
